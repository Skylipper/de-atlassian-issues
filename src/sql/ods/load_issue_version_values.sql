...
WITH last_updated as (SELECT COALESCE(
                                     (SELECT (workflow_settings::jsonb ->> 'last_loaded_ts')::timestamp
                                      ...
                                      WHERE workflow_key = 'ods.issue_version_values'),
...
INSERT
INTO ods.issue_version_values (issue_id, project_id, version_id, version_name, update_ts)
SELECT object_id::int                                               as issue_id,
       (object_value::jsonb -> 'fields' -> 'project' ->> 'id')::int as project_id,
       (jsonb_array_elements_text(object_value::jsonb -> 'fields' -> 'versions')::jsonb ->>
